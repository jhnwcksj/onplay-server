...
router.post('/networks', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Для user и vip-user проверяем лимит на количество сетей (максимум 1)
    if (userRole === 'user' || userRole === 'vip-user') {
      const networksCount = await pool.query(
        'SELECT COUNT(*) FROM network_users WHERE user_id = $1',
        [userId]
      );
      if (parseInt(networksCount.rows[0].count) >= 1) {
        return res.status(403).json({ error: 'Вы можете создать только одну сеть' });
      }
    }
