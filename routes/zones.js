...
// GET /zones - return zones for a branch (?branchId=)
router.get('/zones', async (req, res) => {
  const branchId = req.query.branchId;
  
  // Обязательно требуем branchId для защиты от несанкционированного доступа
  if (!branchId) {
    return res.status(400).json({ error: 'Необходимо указать филиал для получения списка зон' });
  }
  ...
  try {
    ...
    const result = await pool.query(baseSelect + ' WHERE z.branch_id = $1 ORDER BY z.zone_id', [branchId]);
