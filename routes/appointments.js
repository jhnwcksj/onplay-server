...
// Получить записи по услуге (и опционально филиалу) или по диапазону дат
// GET /appointments?service_id=... - ищет по service_id и по services (JSONB в appointment_meta.extra)
// GET /appointments?branchId=...&startDate=...&endDate=... - ищет по диапазону дат
// GET /appointments?branchId=...&startDate=...&endDate=...&service_ids[]=X&service_ids[]=Y - фильтр по услугам
router.get('/appointments', async (req, res) => {
  const { service_id, branch_id, branchId, startDate, endDate } = req.query;
  const service_ids = req.query['service_ids[]']; // Express собирает service_ids[] параметры
  
  // Определяем активный branch_id (поддерживаем оба варианта)
  const activeBranchId = branchId || branch_id;
  
  try {
    // Если указаны startDate и endDate - возвращаем записи за период
    if (startDate && endDate) {
      // Получаем агрегированные данные по датам
      // Конвертируем в локальный часовой пояс (Asia/Almaty) перед извлечением даты
      let query = `
        SELECT 
          TO_CHAR(DATE(a.start_time AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Almaty'), 'YYYY-MM-DD') as date,
          COUNT(*) as count,
          BOOL_OR(
            COALESCE(s.pricing_type = 'package', false) OR 
            COALESCE(sc.pricing_type = 'package', false)
          ) as has_package
        FROM appointments a
        LEFT JOIN services s ON a.service_id = s.service_id
        LEFT JOIN service_categories sc ON s.category_id = sc.category_id
        WHERE DATE(a.start_time AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Almaty') >= $1 
          AND DATE(a.start_time AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Almaty') <= $2
      `;
      ...
      // Добавляем фильтр по service_ids если указан
      if (service_ids && service_ids.length > 0) {
        const serviceIdsArray = Array.isArray(service_ids) ? service_ids : [service_ids];
        query += ` AND a.service_id = ANY($${params.length + 1}::int[])`;
        params.push(serviceIdsArray.map(id => parseInt(id)));
      }
      
      query += ' GROUP BY DATE(a.start_time AT TIME ZONE \'UTC\' AT TIME ZONE \'Asia/Almaty\') ORDER BY date';
      
      const result = await pool.query(query, params);
      return res.json({ dates: result.rows });
    }
    
    // Старая логика - поиск по service_id
    if (!service_id) return res.status(400).json({ error: 'service_id или startDate/endDate required' });
...
router.post('/appointments', async (req, res) => {
  try {
    ...
    // Получаем timezone филиала для корректной интерпретации времени
    let branchTimezone = 'Asia/Almaty'; // значение по умолчанию
    let branchValidUntil = null;
    try {
      const branchRes = await pool.query(
        'SELECT timezone, valid_until FROM branches WHERE branch_id = $1',
        [branch_id]
      );
      if (branchRes.rows.length > 0) {
        if (branchRes.rows[0].timezone) {
          branchTimezone = branchRes.rows[0].timezone;
        }
        branchValidUntil = branchRes.rows[0].valid_until;
      }
    } catch (err) {
      console.warn('Failed to fetch branch data, using default:', err.message);
    }

    // Парсим ISO строки как время в timezone филиала
    // Фронтенд отправляет ISO строки без timezone offset: "2024-01-15T10:00:00"
    // Это значит 10:00 в timezone филиала, не в UTC
    
    const tzOffset = getTimezoneOffsetString(branchTimezone);
    
    // Добавляем timezone offset к ISO строкам
    const startTimeStr = start_time.includes('+') || start_time.includes('Z') 
      ? start_time 
      : start_time + tzOffset;
    const endTimeStr = end_time.includes('+') || end_time.includes('Z')
      ? end_time
      : end_time + tzOffset;
    ...
    // Проверка даты для user/vip-user с ограниченным доступом
    if (req.user && (req.user.role === 'user' || req.user.role === 'vip-user') && branchValidUntil) {
      const appointmentDate = startDateTime.toISOString().split('T')[0]; // YYYY-MM-DD
      if (appointmentDate > branchValidUntil) {
        console.log(`[POST appointments] Correcting date from ${appointmentDate} to ${branchValidUntil} for user ${req.user.id} (role: ${req.user.role})`);
        // Корректируем дату на последний день valid_until
        const validParts = branchValidUntil.split('-').map(Number);
        const correctedStart = new Date(validParts[0], validParts[1] - 1, validParts[2], startDateTime.getHours(), startDateTime.getMinutes(), startDateTime.getSeconds());
        const correctedEnd = new Date(validParts[0], validParts[1] - 1, validParts[2], endDateTime.getHours(), endDateTime.getMinutes(), endDateTime.getSeconds());
        startDateTime.setTime(correctedStart.getTime());
        endDateTime.setTime(correctedEnd.getTime());
      }
    }
...
router.put('/appointments/:id', async (req, res) => {
  try {
    ...
    let branchTimezone = 'Asia/Almaty';
    let branchValidUntil = null;
    try {
      const branchRes = await pool.query(
        'SELECT timezone, valid_until FROM branches WHERE branch_id = $1',
    ...
    if (isNaN(startDateTime.getTime()) || isNaN(endDateTime.getTime())) {
      console.error('Invalid datetime format (PUT):', { 
        start_time, 
        end_time, 
        startTimeStr, 
        endTimeStr,
    ...
    if (req.user && (req.user.role === 'user' || req.user.role === 'vip-user') && branchValidUntil) {
      const appointmentDate = startDateTime.toISOString().split('T')[0]; // YYYY-MM-DD
      if (appointmentDate > branchValidUntil) {
        console.log(`[PUT appointments] Correcting date from ${appointmentDate} to ${branchValidUntil} for user ${req.user.id} (role: ${req.user.role})`);
        // Корректируем дату на последний день valid_until
        const validParts = branchValidUntil.split('-').map(Number);
        const correctedStart = new Date(validParts[0], validParts[1] - 1, validParts[2], startDateTime.getHours(), startDateTime.getMinutes(), startDateTime.getSeconds());
        const correctedEnd = new Date(validParts[0], validParts[1] - 1, validParts[2], endDateTime.getHours(), endDateTime.getMinutes(), endDateTime.getSeconds());
