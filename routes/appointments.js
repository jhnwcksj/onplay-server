...
// Общая функция парсинга даты/времени DD.MM.YYYY + HH:MM в объект Date
...
router.post('/appointments/check', async (req, res) => {
  try {
    const {
      ...
      time_from, // HH:MM
      time_to,   // HH:MM
      // необязательный id текущей записи, чтобы не считать её конфликтом при редактировании
      appointment_id,
    } = req.body || {};
    ...
    const result = await pool.query(
      `SELECT a.id,
        ...
        WHERE a.branch_id = $1
          ...
          AND az.zone_id = ANY($3::bigint[])
          AND a.start_time < $4
          AND a.end_time   > $5
          AND ($6::bigint IS NULL OR a.id <> $6::bigint)
        GROUP BY a.id, a.start_time, a.end_time
        ORDER BY a.start_time`,
      [branch_id, startDateTime, zone_ids, endDateTime, startDateTime, appointment_id || null]
    );
