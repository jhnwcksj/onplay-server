...
router.get('/branches/:branchId/zones/:zoneId/services', async (req, res) => {
  ...
  const weekday = req.query.weekday ? parseInt(req.query.weekday, 10) : null; // день недели 0-6
  ...
  try {
    const result = await db.query(`
      SELECT s.service_id, s.name, s.description, s.duration,
        ...
        COALESCE(sp_override.price, sp_time.price, sp_any.price) AS price,
        (
      ...
      -- Проверяем переопределения дня недели для услуги
      LEFT JOIN LATERAL (
        SELECT override_day_type
        FROM service_week_overrides
        WHERE service_id = s.service_id
          AND weekday = $5
        LIMIT 1
      ) swo ON TRUE
      -- Цена с учетом переопределения
      LEFT JOIN LATERAL (
        SELECT price
        FROM service_prices
        WHERE service_id = s.service_id
          ...
          AND day_type = COALESCE(swo.override_day_type, $3)
        ...
        LIMIT 1
      ) sp_override ON swo.override_day_type IS NOT NULL
      -- Цена по времени без переопределения
      LEFT JOIN LATERAL (
        SELECT price
        FROM service_prices
        WHERE service_id = s.service_id
        ...
        ORDER BY priority DESC, price ASC
        LIMIT 1
      ) sp_time ON swo.override_day_type IS NULL
      -- Цена без времени
      LEFT JOIN LATERAL (
        SELECT price
        FROM service_prices
    ...
    `, [branchId, zoneId, dayType, time, weekday]);
...
// Получить услуги по категории или филиалу
router.get('/services', async (req, res) => {
  const categoryId = req.query.categoryId;
  const branchId = req.query.branch_id || req.query.branchId;
  
  // Обязательно требуем branchId для защиты от несанкционированного доступа
  if (!branchId) {
    return res.status(400).json({ error: 'Необходимо указать филиал для получения списка услуг' });
  }
  
  try {
    // Если передан categoryId, возвращаем услуги конкретной категории этого филиала
    if (categoryId) {
      const result = await db.query(`
        SELECT s.*, sc.name as category_name 
        FROM services s
        JOIN service_categories sc ON s.category_id = sc.category_id
        WHERE s.category_id = $1 AND sc.branch_id = $2 AND s.is_active = TRUE
        ORDER BY s.name
      `, [categoryId, branchId]);
      res.json(result.rows);
    } else {
      // Если categoryId не передан, возвращаем все услуги филиала
      const result = await db.query(`
        SELECT s.*, sc.name as category_name 
        FROM services s
        JOIN service_categories sc ON s.category_id = sc.category_id
        WHERE sc.branch_id = $1 AND s.is_active = TRUE
        ORDER BY sc.name, s.name
      `, [branchId]);
      res.json(result.rows);
    }
...
// === Week Overrides для услуг ===

// Получить week overrides для услуги
router.get('/services/:serviceId/week-overrides', async (req, res) => {
  const { serviceId } = req.params;
  
  try {
    const result = await db.query(
      `SELECT id, service_id, weekday, override_day_type, created_at
       FROM service_week_overrides
       WHERE service_id = $1
       ORDER BY weekday`,
      [serviceId]
    ...
    console.error('Error fetching week overrides:', err);
    res.status(500).json({ error: err.message });
  }
});

// Установить/обновить week override для услуги и конкретного дня недели
router.post('/services/:serviceId/week-overrides', async (req, res) => {
  const { serviceId } = req.params;
  const { weekday, override_day_type } = req.body;
  
  if (weekday === undefined || weekday === null) {
    return res.status(400).json({ error: 'weekday is required (0-6)' });
  }
  
  if (!override_day_type || !['weekday', 'weekend', 'holiday'].includes(override_day_type)) {
    return res.status(400).json({ error: 'override_day_type must be weekday, weekend, or holiday' });
  }
  
  try {
    // Используем UPSERT (ON CONFLICT DO UPDATE)
    const result = await db.query(
      `INSERT INTO service_week_overrides (service_id, weekday, override_day_type)
       VALUES ($1, $2, $3)
       ON CONFLICT (service_id, weekday)
       DO UPDATE SET override_day_type = EXCLUDED.override_day_type
       RETURNING *`,
      [serviceId, weekday, override_day_type]
    );
    
    res.json({ success: true, override: result.rows[0] });
  } catch (err) {
    console.error('Error setting week override:', err);
    res.status(500).json({ error: err.message });
  }
});

// Удалить week override для услуги и конкретного дня недели
router.delete('/services/:serviceId/week-overrides/:weekday', async (req, res) => {
  const { serviceId, weekday } = req.params;
  
  try {
    const result = await db.query(
      'DELETE FROM service_week_overrides WHERE service_id = $1 AND weekday = $2 RETURNING *',
      [serviceId, weekday]
    );
    ...
    console.error('Error deleting week override:', err);
    res.status(500).json({ error: err.message });
  }
});
